...
#define IRQ_IDX_GYRO_CAL_X                                                     \
  (IRQ_IDX_STREAM_SENSOR_QUEUE + STREAM_SENSOR_QUEUE_SIZE)
...
#define SH_TO_H32(buf)                                                         \
  (int32_t)(*(buf) << 24 | *((buf) + 1) << 16 | *((buf) + 2) << 8 |            \
            *((buf) + 3))
...
int stml0xx_blink_set(struct led_classdev *led_cdev, unsigned long *delay_on,
